...
def cafe_run(env, servers, _lambda, _mu):
    ...
    Args:
        ...
        _lambda (float): the average inter-arrival period (minutes)
...
for i in range(NUM_RUNS):
    ...
    if NUM_RUNS <= 1:
        ...
        plt.figure()
        plt.plot(1+np.arange(len(queue_wait)), 
                 [np.mean(queue_wait[0:i]) for i in range(len(queue_wait))], 
                  label='Wait in Queue')
        plt.plot(1+np.arange(len(total_wait)), 
                 [np.mean(total_wait[0:i]) for i in range(len(total_wait))], 
                  label='Total Wait')
        plt.xlabel('Customer')
